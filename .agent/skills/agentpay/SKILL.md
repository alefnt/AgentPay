...
2. Caller pays → funds are **locked** on Fiber (not yet settled)
3. Provider executes the task
4. Provider reveals `preimage` → funds **settle** (released to provider)
5. If task fails → Provider cancels invoice → funds **return** to caller
